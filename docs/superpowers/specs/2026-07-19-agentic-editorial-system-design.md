...
The workflow's required gates use only `schema`, `scope-duplication`, `evidence`, `template`, `wordpress`, `discovery`, and `human`. Each human-only edge requires `human`; final handoff and human approval accumulate the relevant earlier gates. A passing validation report must name the exact edge and actor, contain exactly one passing result for each required gate with no duplicate identifiers or extras, and name the workflow edge's exact next allowed role. The writer's `research_accepted` to `drafting` report is preflight evidence produced before WordPress mutation, so that edge requires `schema`, `scope-duplication`, `evidence`, and `template`. The `wordpress` gate is post-draft and begins on `drafting` to `technical_verification`.

Bounded automation attempts are available only to roles declared in the manifest. Every attempt preserves `kind`, `actor`, `from_state`, and `to_state`, and uses `wordpress_operation` with exactly `none`, `create-draft`, or `update-draft`. It also contains a strict `validation_report_ref` with only a safe repository-relative POSIX JSON path in the `editorial/` validation-report artifact namespace and a lowercase SHA-256 of the exact report file bytes; arbitrary JSON paths, traversal, symbolic-link escape, JSON reserialization, whitespace normalization, and key sorting are rejected. The hash binds repository-stored bytes for integrity and is not an authenticity signature. Validation resolves that immutable report and cross-checks contract `1.0.0`, canonical slug, report role against attempt actor, the exact edge, passing status, exactly the required passing gates, and the exact next role. The attempt cannot authorize itself with a scalar status or an unrelated report.
