...
/**
 * Reports whether a product-home page has been promoted through Reading Settings.
 *
...
/**
 * Prevents product-home previews from being indexed before promotion.
 *
...
function mumega_motion_filter_product_home_robots( $robots ) {
	if ( ! is_page_template( 'page-templates/product-home.php' ) || mumega_motion_is_promoted_product_home() ) {
		return $robots;
	}
...
/**
 * Returns published product-home page IDs that are still previews.
 *
...
/**
 * Adds unpromoted product-home pages to Yoast's sitemap exclusion IDs.
 *
...
/**
 * Removes an unpromoted product-home page entry from Rank Math sitemaps.
 *
...
function mumega_motion_filter_rank_math_sitemap_entry( $url, $type, $sitemap_object ) {
	...
	if ( 'page-templates/product-home.php' !== get_page_template_slug( $post_id ) || mumega_motion_is_promoted_product_home( $post_id ) ) {
		return $url;
	}
